interface FromMarkdownOptions {
    ...
    wikiImgLinkClassName?: string;
    hrefTemplate?: (permalink: string) => string;
}

function fromMarkdownWikiImgLink(opts: FromMarkdownOptions = {}) {
    ...
    const wikiImgLinkClassName = opts.wikiImgLinkClassName || "internal";
    const defaultHrefTemplate = (permalink: string) => `#/page/${permalink}`;
    const hrefTemplate = opts.hrefTemplate || defaultHrefTemplate;
    let node: any;

    function enterWikiImgLink(this: any, token: any) {
        node = {
            type: "wikiImgLink",
            value: null,
            data: {
    ...
    function exitWikiImgLinkAlias(this: any, token: any) {
        const alias = this.sliceSerialize(token);
        const current = top(this.stack);
        current.data.alias = alias;
    }

    function exitWikiImgLinkTarget(this: any, token: any) {
        const target = this.sliceSerialize(token);
        const current = top(this.stack);
        current.value = target;
    }

    function exitWikiImgLink(this: any, token: any) {
        this.exit(token);
        const wikiLink = node;
        ...
        let classNames = wikiImgLinkClassName;
        if (!exists) {
    ...
    return {
        enter: {
            wikiImgLink: enterWikiImgLink,
        },
        exit: {
            wikiImgLinkTarget: exitWikiImgLinkTarget,
            wikiImgLinkAlias: exitWikiImgLinkAlias,
            wikiImgLink: exitWikiImgLink,
        },
    };
}

function fromMarkdownWikiLink(opts: FromMarkdownOptions = {}) {
    const permalinks = opts.permalinks || [];
    const defaultPageResolver = (name: string) => [
        name.replace(/ /g, "_").toLowerCase(),
    ];
...
export { fromMarkdownWikiImgLink, fromMarkdownWikiLink };
